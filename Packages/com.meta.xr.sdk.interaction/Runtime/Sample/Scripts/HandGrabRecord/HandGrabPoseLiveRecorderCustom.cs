...
{
    public class HandGrabPoseLiveRecorderCustom : MonoBehaviour, IActiveState
    {
        [SerializeField]
        ...
        private void HideAllRecordedPoses()
        {
            foreach (var step in _recorderSteps)
            {
                if (step.interactable != null)
                {
                    step.interactable.gameObject.SetActive(false);
                }
            }
        }

        private void ShowAllRecordedPoses()
        {
            foreach (var step in _recorderSteps)
            {
                if (step.interactable != null)
                {
                    step.interactable.gameObject.SetActive(true); 
                }
            }
        }
        ...
        private bool TakeSnapshot()
        {
            ...
            Debug.Log($"Left item: {leftItem?.name}, Right item: {rightItem?.name}");
            Debug.Log($"Left distance: {leftDistance}, Right distance: {rightDistance}");
            if (leftDistance < rightDistance
                && leftItem != null)
            {
                Debug.Log($"Left item: {leftItem.name}");
                return Record(_leftHand.Hand, leftItem);
            }
            else if (rightItem != null)
            {
                Debug.Log($"Right item: {rightItem.name}");
        ...
        private bool Record(IHand hand, Rigidbody item)
        {
            ...
            SetThumbIndexLockedOthersFree(point);

            return true;
        }

        private void SetThumbIndexLockedOthersFree(HandGrabPose handGrabPose)
        {
            handGrabPose.HandPose.FingersFreedom = new JointFreedom[Constants.NUM_FINGERS]
            {
                JointFreedom.Locked,
                JointFreedom.Locked,
                JointFreedom.Free,
    ...
    public class RigidbodyDetectorCustom : MonoBehaviour
    {
